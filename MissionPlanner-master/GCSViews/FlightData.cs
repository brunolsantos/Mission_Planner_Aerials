...
namespace MissionPlanner.GCSViews
{
    ...
    {
        bool imagemCamera = false;
        bool adsb_Flag = false;
        bool paraquedas_Flag = false;
        vlcrender render = new vlcrender();
        ...
        public void loadFullscreenImage()
        {
            Bitmap fullscreen = new Bitmap(global::MissionPlanner.Properties.Resources.fullscreen);
            fullscreenToolStripMenuItem.Image = new Bitmap(fullscreen, 25, 25);
            fullscreen.Dispose();
        }
        public FlightData()
        {
            ...
            InitializeComponent();
            this.configuracoesToolStripMenuItem.ToolTipText = "Configuration";
            horizontalMenuMap.Parent = gMapControl1;
            verticalMenuMap.Parent = gMapControl1;
            takeoff_option.Parent = gMapControl1;
            menuConnect.Parent = gMapControl1;
            loadFullscreenImage();

            ensureDisabled(27, "RC12");
            ensureDisabled(55, "RC12");
            MainV2.comPort.setParam("RC12_FUNCTION", 55);
            ...
            //Manual, FBWA, Loiter, Auto, Stabilized e Land
            /*CMB_modes.Items.Remove("CIRCLE");
            CMB_modes.Items.Remove("TRAINING");
            CMB_modes.Items.Remove("ACRO");
            CMB_modes.Items.Remove("FBWB");
            CMB_modes.Items.Remove("CRUIZE");
            CMB_modes.Items.Remove("AUTOTUNE");
            CMB_modes.Items.Remove("RTL");
            CMB_modes.Items.Remove("Guided");
            CMB_modes.Items.Remove("INITIALISING");
            CMB_modes.Items.Remove("QStabilize");
            CMB_modes.Items.Remove("QHover");*/
            ...
            QRTL
             */
            //AUTO, LOITER, LAND e CRUISE
            CMB_modes.Items.Remove("Manual");
            CMB_modes.Items.Remove("CIRCLE");
            CMB_modes.Items.Remove("STABILIZE");
            CMB_modes.Items.Remove("TRAINING");
            CMB_modes.Items.Remove("ACRO");
            CMB_modes.Items.Remove("FBWA");
            CMB_modes.Items.Remove("FBWB");
            CMB_modes.Items.Remove("AUTOTUNE");
            CMB_modes.Items.Remove("Guided");
            CMB_modes.Items.Remove("INITIALISING");
            CMB_modes.Items.Remove("QStabilize");
            CMB_modes.Items.Remove("QHover");
            CMB_modes.Items.Remove("QLoiter");
            CMB_modes.Items.Remove("QLand");
            CMB_modes.Items.Remove("QRTL");
        ...
        {
            contextMenuStripHud.Items[0].Visible = false;
            contextMenuStripHud.Items[1].Visible = false;
            contextMenuStripHud.Items[3].Visible = false;

            contextMenuStripMap.Items[2].Visible = false;
            contextMenuStripMap.Items[3].Visible = false;
            contextMenuStripMap.Items[4].Visible = false;
            contextMenuStripMap.Items[5].Visible = false;
            contextMenuStripMap.Items[6].Visible = false;
        }

        public void setMenuItemsToolTip()
        {
        ...
        private void MainV2_AdvancedChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                tabControlactions.TabPages.Remove(tabActionsSimple);
                tabControlactions.TabPages.Remove(tabGauges);
                //tabControlactions.TabPages.Remove(tabTLogs); //telemetry logs
                //tabControlactions.TabPages.Remove(tablogbrowse);//dataflash logs
                tabControlactions.TabPages.Remove(tabScripts);
        ...
        private void mainloop()
        {
            ...
            while (threadrun)
            {
                ...
                //reconnectCamera();//tentativa de reconectar camera
            }
            Console.WriteLine("FD Main loop exit");
        }
        public void reconnectCamera()
        {
            if ((imagemCamera == true) && (CaptureMJPEG.getRunningStatus() == false))
            {
                try
                {
                    CaptureMJPEG.Stop();
                ...
                catch (Exception e) { MessageBox.Show("ERRO:\n\n" + e); }
            }
        }
        ...
        private void BUT_loadtelem_Click(object sender, EventArgs e)
        {
            ...
            {
                Settings.Instance.LogDir = Path.GetDirectoryName(Application.ExecutablePath) + Path.DirectorySeparatorChar + @"logs";
        ...
        public void captureVideo()
        {
            try
            {
                stopRecordToolStripMenuItem_doClick();
                //rtsp://admin:@10.50.0.12/user=admin_password=tlJwpbo6_channel=1_stream=0.sdp
                //CustomMessageBox.Show("Output avi will be saved to the log folder");

                aviwriter = new AviWriter();
                string pathDirectory = Path.GetDirectoryName(Application.ExecutablePath) + Path.DirectorySeparatorChar + @"logs";
                //Directory.CreateDirectory(Settings.Instance.LogDir);
                Directory.CreateDirectory(pathDirectory);
                //aviwriter.avi_start(Settings.Instance.LogDir + Path.DirectorySeparatorChar + DateTime.Now.ToString("yyyy-MM-dd HH-mm-ss") + ".avi");
                aviwriter.avi_start(pathDirectory + Path.DirectorySeparatorChar + DateTime.Now.ToString("yyyy-MM-dd HH-mm-ss") + ".avi");

                recordHudToAVIToolStripMenuItem.Text = "Recording";
            }
            catch (Exception exc)
            {
                CustomMessageBox.Show(Strings.ERROR + " " + exc, Strings.ERROR);
            }
        }

        public void stopRecordToolStripMenuItem_doClick()
        {
        ...
        private void BUT_Antena_Click(object sender, EventArgs e)
        {
            //temp temp = new temp();
            //temp.BUT_outputMavlink_doClick();
            SerialOutputPass serial = new SerialOutputPass();
            ThemeManager.ApplyThemeTo(serial);
            serial.Show();
        }

        private void gravarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (imagemCamera == false)
            {
                render.playurl = MainV2.instance.getVideoURL();
                try
                {
                    render.Start();
                    captureVideo();
                    gravarToolStripMenuItem.Image = global::MissionPlanner.Properties.Resources.video_Off;
                }
                catch (Exception ex)
                {
            ...
            else
            {
                ...
                imagemCamera = false;
                stopRecordToolStripMenuItem_doClick();
                gravarToolStripMenuItem.ToolTipText = "Iniciar gravação";
                //CaptureMJPEG.Stop();
                gravarToolStripMenuItem.Image = global::MissionPlanner.Properties.Resources.video_On;
            }
        }

        private void rtlToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (CustomMessageBox.Show("Are you sure you want to do RETURN_TO_LAUNCH ?", "Action", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                try
                {
                    ...
                    rtlToolStripMenuItem.Image = global::MissionPlanner.Properties.Resources.rtl_NewButton_Vermelho;
                    MainV2.comPort.doCommand((MAVLink.MAV_CMD)Enum.Parse(typeof(MAVLink.MAV_CMD), "RETURN_TO_LAUNCH"), param1, 0, param3, 0, 0, 0, 0);

                }
                catch
                {
                    rtlToolStripMenuItem.Image = global::MissionPlanner.Properties.Resources.rtl_NewButton;
                    CustomMessageBox.Show(Strings.CommandFailed, Strings.ERROR);
                }
        ...
        public void setRtlWhiteButton()
        {
            rtlToolStripMenuItem.Image = global::MissionPlanner.Properties.Resources.rtl_NewButton;
        }

        private void geoRefImagensToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Georefimage geo = new Georefimage();
            ThemeManager.ApplyThemeTo(geo);
            geo.Show();
            //ThemeManager.ApplyThemeTo();
            //new GeoRef.Georefimage().Show();

        }

        private void configuracoesIniciaisToolStripMenuItem_Click(object sender, EventArgs e)
        {
            MainV2.instance.MenuInitConfig_doClick();
        }

        private void configuracoesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            MainV2.instance.MenuConfigTune_doClick();
        }

        private void PlanoDeVooToolStripMenuItem_Click(object sender, EventArgs e)
        {
            MainV2.instance.MenuFlightPlanner_doClick();
        }
        private void conectarToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            MainV2.instance.setMenuEnable();
        }
        ...
        private void fullscreenToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            MainV2.instance.fullScreenToolStripMenuItem_doClick();
        }
        ...
        private void paraquedasToolStripMenuItem_Click(object sender, EventArgs e)
        {
            /*
             ...
             */
            //MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_VTOL_TRANSITION, 0, 0, 0, 0, 0, 0, 0);
            ///<summary> Cycle a between its nominal setting and a desired PWM for a desired number of cycles with a desired period. |Servo number| PWM (microseconds, 1000 to 2000 typical)| Cycle count| Cycle time (seconds)| Empty| Empty| Empty|  </summary>
                try
                {
                    ...
                    if (MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 10, 2000, 0, 0, 0, 0, 0))
                    {
                    }
                    else
                    {
            ...
            t.Interval = 1; //In milliseconds here
            t.AutoReset = false; //Stops it from repeating
            t.Elapsed += new ElapsedEventHandler(turnServoParachuteOn);
            t.Start();

            //servo = 5
            /*string desc = Settings.Instance["Servo" + 5 + "_desc"];
            string low = Settings.Instance["Servo" + 5 + "_low"];
            string high = Settings.Instance["Servo" + 5 + "_high"];

            string highdesc = Settings.Instance["Servo" + 5 + "_highdesc"];
            string lowdesc = Settings.Instance["Servo" + 5 + "_lowdesc"];
            int microssegundos = int.Parse(low);
            //microssegundos = PWM (microseconds, 1000 to 2000 typical)
            MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 5, microssegundos, 0, 0, 0, 0, 0);
            MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_PARACHUTE, (int)MAVLink.PARACHUTE_ACTION.PARACHUTE_ENABLE, 0, 0, 0, 0, 0, 0);*/
        }
        void turnServoParachuteOn(object sender, ElapsedEventArgs e)
        {
            try
            {
                ...
                //|Servo number| PWM (microseconds, 1000 to 2000 typical)| Cycle count| Cycle time (seconds)| Empty| Empty| Empty|
                //ensureDisabled(55, "RC12");
                ///MainV2.comPort.setParam("RC12_FUNCTION", 0);
                //MainV2.comPort.setParam("RC12_FUNCTION", 55);

                ///MainV2.comPort.setParam("RC12_FUNCTION", 27);
                //MainV2.comPort.setParam("RC12_FUNCTION", 27);
                ///MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_PARACHUTE, 2, 0, 0, 0, 0, 0, 0);
                //ensureDisabled(27, "RC12");
                ///MainV2.comPort.setParam("RC12_FUNCTION", 0);
                ///MainV2.comPort.setParam("RC12_FUNCTION", 55);
                MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 10, 1100, 1, 2, 0, 0, 0);
                MessageBox.Show("ligando paraquedas");
                paraquedasToolStripMenuItem.Image = global::MissionPlanner.Properties.Resources.parachute_Open;

                /*System.Timers.Timer t = new System.Timers.Timer();
                t.Interval = 15000; //In milliseconds here
                t.AutoReset = false; //Stops it from repeating
                t.Elapsed += new ElapsedEventHandler(turnServoParachuteOff);
                t.Start();  */
        ...
        void turnServoParachuteOff(object sender, ElapsedEventArgs e)
        {
            try
            {
                //|Servo number| PWM (microseconds, 1000 to 2000 typical)| Cycle count| Cycle time (seconds)| Empty| Empty| Empty|
                ...
                //MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_PARACHUTE, 0, 0, 0, 0, 0, 0, 0);
                MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 12, 2000, 1, 2, 0, 0, 0);
                ensureDisabled(27, "RC12");
                MainV2.comPort.setParam("RC12_FUNCTION", 55);
                paraquedasToolStripMenuItem.Image = global::MissionPlanner.Properties.Resources.parachute_Closed;
                paraquedas_Flag = true;
            }
            catch (Exception ex)
            {
                CustomMessageBox.Show(Strings.CommandFailed + ex.ToString(), Strings.ERROR);
        ...
        private void ensureDisabled(int number, string exclude = "")
        {

            if (MainV2.comPort.MAV.param.ContainsKey("RC12_FUNCTION"))
            {
                var ans = (float)MainV2.comPort.MAV.param["RC12_FUNCTION"];
        ...
        private void adsbToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (adsb_Flag == false)
            {
        ...
        {
            CMB_action.Text = "PREFLIGHT_CALIBRATION";
            if (
                CustomMessageBox.Show("Are you sure you want to do " + CMB_action.Text + " ?", "Action",
                    MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
        ...
        private void catapultaToolStripMenuItem_Click(object sender, EventArgs e)
        {

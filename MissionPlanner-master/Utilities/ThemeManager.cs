...
namespace MissionPlanner.Utilities
{
    ...
    public class ThemeManager
    {
        ...
        public static Color BGColor = Color.FromArgb(217, 217, 217);
        public static Color ControlBGColor = Color.White; //Color.FromArgb(46, 134, 222);
        public static Color TextColor = Color.Black;
        public static Color ButBG = Color.FromArgb(46, 134, 222);//ButBG;
        public static Color ButBorder = Color.FromArgb(46, 134, 222);
        ...
        public static void SetTheme(Themes theme)
        {
            ...
            //_currentTheme = theme;//original program
            _currentTheme = Themes.Custom;//modified
        }
...
> <Grid>";
        ...
        private static void ApplyCustomTheme(Control temp, int level)
        {
            ...
            foreach (Control ctl in temp.Controls)
            {
                ...
                else if (ctl.GetType() == typeof(MyButton))
                {
                    ...
                    try
                    {
                        but.BGGradBot = Color.FromArgb(46, 134, 222);//Color.FromArgb(ButBG.ToArgb() - 0x333333);
                    }
                    catch
                    {
                    }
                    //but.TextColor = TextColor; //Original
                    but.TextColor = Color.White;//White color
                    but.Font = new Font(but.Font.Name, but.Font.Size, FontStyle.Bold);//Bold style
                ...
                else if (ctl.GetType() == typeof(DataGridView))
                {
                    ...
                    hs.BackColor = ControlBGColor;//BGColor;
        ...
        private static void ApplyHighContrast(Control temp, int level)
        {
            ...
            foreach (Control ctl in temp.Controls)
            {
                ...
                {
                    ctl.ForeColor = Color.Black;//TextColor;
                    DataGridView dgv = (DataGridView)ctl;
                    dgv.EnableHeadersVisualStyles = false;
                    dgv.BorderStyle = BorderStyle.None;
                    dgv.BackgroundColor = BGColor;
                    DataGridViewCellStyle rs = new DataGridViewCellStyle();
                    rs.BackColor = Color.White;//ControlBGColor;
                    rs.ForeColor = Color.Black;//TextColor;
                    dgv.RowsDefaultCellStyle = rs;

                    dgv.AlternatingRowsDefaultCellStyle = rs;

                    DataGridViewCellStyle hs = new DataGridViewCellStyle(dgv.ColumnHeadersDefaultCellStyle);
                    hs.BackColor = Color.White;//BGColor;
                    hs.ForeColor = Color.Black;//TextColor;
